...
class SlackNotifier:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(self.webhook_url, json=payload) as response:
                    ...
                    if response.status == 200:
                        ...
                        logger.error(
                            f"❌ Failed to send Slack notification. Status: {response.status}, Response: {await response.text()}"
                        )

        except Exception as e:
            logger.error(
                f"❌ Exception occurred while sending Slack notification: {str(e)}"
            )
    ...
    async def send_system_alert(self, component: str, status: str, details: str = ""):
        ...
        message = (
            f"{emoji} *System Alert: {component}*\nStatus: {status}\nDetails: {details}"
        )
