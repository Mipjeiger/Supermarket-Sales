...
from app.api.dependencies import get_abuse_agent, get_llm_judge
...
async def get_abuse_status(
    ...
    """
    Get abuse investigation status for a user.
    """
    try:
        status = await abuse_agent.get_investigation_status(user_id)
        return status
    except Exception as e:
