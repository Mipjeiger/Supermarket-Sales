...
class SecurityStreamRequest(BaseModel):
    streaming_db_row: Dict[str, Any] = Field(..., description="Real-time transaction features corresponding to your database columns")
    risk_metrics: Dict[str, Any] = Field(..., description="Upstream risk context elements, including baseline pipeline abuse velocity scores")
...
async def analyze_stream(payload: SecurityStreamRequest):
    ...
    try:
        if not payload.streaming_db_row:
            raise HTTPException(status_code=400, detail="Streaming transaction data payload is empty.")
