...
class FraudInvestigationRequest(BaseModel):
    ...
    order_id: Optional[str] = Field(
        "ID-2012-79922", description="Unique identifier for the supermarket order."
    )
    customer_name: str = Field(
        ..., example="John Lee", description="Target customer name."
    )
    investigation_date: str = Field(
        "2012-12-10", example="2026-07-25", description="Audit or evaluation date."
    )
    category: str = Field(
    ...
    unit_price: float = Field(
        75000.0, example=75000.0, description="Price per individual unit."
    )
    ...
    profit_margin: float = Field(
        -0.07, example=-0.07, description="Profit margin ratio."
    )
    shipping_days: int = Field(
        214, example=214, description="Days taken to ship the order."
    )
    order_priority: Optional[str] = Field(
        "Low", example="Low", description="Priority level of the order."
    )
    fraud_flag: int = Field(
        0,
        example=0,
        description="Binary classification flag (1 = Fraud, 0 = Legitimate).",
    )
    ...
    abuse_score: float = Field(
        0.04, example=0.04, description="Calculated velocity abuse score."
    )
    custom_context: Optional[str] = Field(
        None, example="High shipping delay with negative profit margin."
    )
