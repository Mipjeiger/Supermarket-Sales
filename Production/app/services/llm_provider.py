...
class LLMProvider:
    ...
    def query_huggingface_chat(
        ...
        if response.status_code == 429:
            raise ResourceWarning(
                "Hugging Face API rate limit exceeded. Please retry after some time."
            )
    ...
    def query_groq_chat(
        ...
        response = requests.post(
            self.groq_url, headers=headers, json=payload, timeout=15
        )
    ...
    def generate_grounded_text(
        ...
        try:
            logger.info(
                f"Sending context payload to Hugging Face serverless layer model: {hf_model}"
            )
            ...
            logger.warning(
                f"⚠️ Hugging Face pipeline restricted or exhausted. Error: {str(e)}"
            )
            logger.info(
                f"🔄 Executing failover sequence. Routing directly to Groq hardware clusters ({groq_model})..."
            )
            ...
            try:
                ...
                logger.error(
                    f"❌ Complete pipeline blackout. Both LLM providers exhausted: {str(critical_error)}"
                )
                raise RuntimeError(
                    f"🚨 System blacked out. Provider fallback failure chain: {str(critical_error)}"
                )
