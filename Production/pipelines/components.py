...
def data_ingestion_and_split(
    dataset_path: str,
    fraud_dataset: Output[Dataset],
    sales_dataset: Output[Dataset]
):
...
def train_sales_cbr_model(
    ...
    model = CatBoostRegressor(iterations=1000, depth=5, learning_rate=0.1, loss_function='RMSE', verbose=0)
    model.fit(X_train, y_train)
